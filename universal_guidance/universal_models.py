...
scbpnet_guided_path = os.path.join(file_dir, '..', '..','re_design','scBPNet')
if scbpnet_guided_path not in sys.path:
    sys.path.append(scbpnet_guided_path)

from scbpnet.models import RegularizedDynamicBPNetProfileCountsConv, scBPnetProfileCounts
...
class scBPnetGuide:
    def __init__(self, checkpoint_path = None):
        ...
        self.predictor_model = scBPnetProfileCounts(profile_model, debug_strand=0, name= None)

        if checkpoint_path:
            self.predictor_model.load_state_dict(torch.load(checkpoint_path, map_location='cpu')) # gpu allocation later by accelerator
        self.predictor_model.eval()
        for param in self.predictor_model.parameters():
            param.requires_grad = False

    
    def __call__(self, X, label_vector):      
        cell_states, counts_target  = label_vector[:,:50], label_vector[:,50:]
        # X : shape (batch_size, 1, 4, 512)
        # cell_states : shape (batch_size, 50)
        # counts_target [counts_target_pos, counts_target_neg]  : shape (batch_size, 2)

        _, y_counts = self.predictor_model(X.squeeze(1), cell_states)
        # y_counts : shape (batch_size, 2)
        per_example_loss = log1pMSELoss_batch(y_counts, counts_target)

        return per_example_loss

def log1pMSELoss_batch(log_predicted_counts, true_counts):
    """A MSE loss on the log(x+1) of the inputs, calculated per example."""
    log_true = torch.log(true_counts + 1)
    # log_predicted_counts shape (batch_size, 2)
    per_example_loss = torch.square(log_true - log_predicted_counts).mean(dim=1)
...
def create_model_and_diffusion(config):
        ...
        if attention_resolutions == "":
            if image_size == 200:
                ...
                attention_resolutions="128,256"
            elif image_size == 1024:
                attention_resolutions="256,512"
            else:
                raise ValueError(f"unsupported seq length: {image_size}")
        attention_ds = []
        ...
        diffusion = SpacedDiffusion(
            ...
            betas=betas,
            model_mean_type=(
                ModelMeanType.EPSILON if not config.predict_xstart else ModelMeanType.START_X
            ),
            model_var_type=(
                (
...
class GaussianDiffusion:
    ...
    def ddim_sample_loop_operation(
        ...
        for sample in self.ddim_sample_loop_progressive_operation(
            ...
            final = sample["sample"]
            losses.append(sample["guide_loss_avg"])
        result_loss = sum(losses)/len(losses) if losses != 0 else 0
        return final, result_loss
    ...
    def ddim_sample_operation(
        ...
        for j in range(num_steps):
            ...
            if operation.guidance_3:
                with torch.enable_grad():
                    ...
                    # pred_xstart.clamp(-1, 1) was useless as it is not inplace
                    # pred_xstart_rescaled = (pred_xstart - pred_xstart.min()) / (pred_xstart.max() - pred_xstart.min())
                    selected = -1 * criterion(pred_xstart.clamp(0, 1), operated_image)
                    ...
                    guide_loss_return.append(torch.sqrt(-1 * selected.detach()).mean().item())
            ...
            for _ in range(max_iters):
                ...
                op_im = (x0 + 1) / 2 #x0.clamp(0,1)
                loss = criterion(op_im, operated_image) 
                # guide_loss_return.append(loss.sum().item())
                ...
                #penalty = torch.mean(F.relu(x0 - 1) + F.relu(-x0 - 1)) # penalizes values outside -1,1
                m_loss = loss.sum() #+ 0.1 * penalty
        ...
        guide_loss_return_ = sum(guide_loss_return)/len(guide_loss_return) if len(guide_loss_return)>0 else 0
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "guide_loss_avg": guide_loss_return_}
...
class EMA:
    ...
    def update_model_average(self, ma_model: torch.nn.Module, current_model: torch.nn.Module) -> None:
    ...
    def step_ema(self, ema_model: torch.nn.Module, model: torch.nn.Module, step_start_ema: int = 500) -> None:
...
def get_tensor_stats(tensor):
    return {"max": torch.max(tensor).item(), "min": torch.min(tensor).item(), "median": torch.median(tensor).item()}
