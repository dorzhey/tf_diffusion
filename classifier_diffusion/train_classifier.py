...
class_weights = compute_class_weight('balanced', classes=th.unique(data['x_train_cell_type']).numpy(), y=data['x_train_cell_type'].numpy())
cell_type_weights = th.tensor(class_weights).float().to(dist_util.dev())
...
def forward_backward_log(data_loader, prefix="train"):
    ...
    for i, (sub_batch, sub_labels, sub_t) in enumerate(
        ...
        loss = F.cross_entropy(logits, sub_labels, reduction="none", weight=cell_type_weights)
        # loss = loss_fn(logits, sub_labels)
        # sub_labels_one_hot = F.one_hot(sub_labels, num_classes=3).float()
        # loss = sigmoid_focal_loss(logits, sub_labels_one_hot, reduction="none").mean(dim=-1)
        ...
        all_targets.extend(sub_labels.cpu().numpy())
        all_preds.extend(th.argmax(logits, dim=1).cpu().numpy())
    print(prefix,"gt", sorted(Counter(all_targets).items()), 'pred', sorted(Counter(all_preds).items()))
    metrics = {}
    metrics[f"{prefix}_precision"] = precision_score(all_targets, all_preds, average='weighted', zero_division=0)
    metrics[f"{prefix}_recall"] = recall_score(all_targets, all_preds, average='weighted', zero_division=0)
    metrics[f"{prefix}_f1"] = f1_score(all_targets, all_preds, average='weighted', zero_division=0)
